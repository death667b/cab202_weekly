...
void cleanup(int count) {
	...
	for (int i = 0; i < count; i++) {
		char buffer[50];
		sprintf(buffer, "Message Iteration #%d\n", i);
		bool test = print_message(buffer,i);
	}
...
int score_count(void) {
	int s = get_current_time();
	if (print_message("Scored", s)) {
		return s % 10;
	}
}
bool print_message(char * msg, int i) {
	printf("%s", msg);
	if (i % 2 == 0) {
		return true;
	}
